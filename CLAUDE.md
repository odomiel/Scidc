...
**DWZ–FIDE merge policy:** A DWZ player is merged with an existing FIDE player entry **iff** both share the same non-zero FIDE ID. No name comparison is performed — FIDE IDs are permanent unique identifiers and are the sole merge key. DWZ players without a FIDE ID (field = `0`) always get their own dictionary entry. Note: `sys::utf8::Codec::matchAscii` and `matchGerman` both require their **second** argument to be 7-bit ASCII (`M_REQUIRE(ascii.is_7bit())`); DWZ names may contain Latin-1 umlauts and must therefore be passed as the first (utf8) argument if these functions are ever used for DWZ name comparisons.
