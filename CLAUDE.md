...
**`strongTtk` for dark mode:** Plain `tk::frame`, `tk::button`, `tk::label` etc. are not styled by TTK — they use the Tk option database. `theme.tcl::SetupCurrentTheme` sets `strongTtk = true` automatically when `currentTheme eq "darkmode"`, which triggers `option add` for all classic Tk widget classes. The full set that must be covered: `*Frame`, `*Toplevel`, `*Dialog`, `*Label`, `*Canvas`, `*Notebook`, `*Button`, `*Checkbutton`, `*Radiobutton`, `*Scrollbar`, **`*Menu`**, **`*Menubutton`**, **`*Text`**, **`*Listbox`**. `tk::menu` widgets (used by every dropdown in the app) are not TTK — omitting `*Menu.background` leaves every menu with the light `#efefef` default even in night mode. This block must run **before** the probe-frame `[$f cget -background]` call so `Settings(tk:background)` captures the dark value.

**`table::ColorLookup` and column header colors:** `tcl/widgets/table.tcl` has a namespace-level `array set ColorLookup` initialized at load time with hardcoded light defaults (`labelforeground black`, `labelbackground #d9d9d9`, etc.). This array is **never updated from `::colors::Colors`** at runtime. `scrolledtable.tcl` passes `-labelbackground theme,background` when building tables — `table::lookupColor` cannot resolve `"theme,background"` (not a key in `ColorLookup`) and returns the string as-is, which treectrl ignores and uses a system default. `ThemeChanged` in `table.tcl` is responsible for fixing this: it calls `$table.t column configure $id -background $bg` (TTK background) **and** `-textcolor $fg` (TTK foreground). Both must be updated together — omitting `-textcolor` leaves column header text black on a `#2b2b2b` background, making headers invisible in dark mode.
