...
static bool
readList(mstl::istream& strm, mstl::string s)
{
	s.clear();
...
static bool
getList(mstl::string& result, char const*& s)
{
	while (::isspace(*s))
		++s;
...
static bool
getToken(mstl::string& result, char const*& s)
{
	while (::isspace(*s))
		++s;
...
checkHashKeys()
{
	...
	for (unsigned i = 0; i < U_NUMBER_OF(Random64); ++i)
	{
		if (!used.insert_unique(Random64[i]))
			fprintf(stderr, "hash key collision: %u\n", i);
	}
	...
	for (unsigned c = 0; c <= 1; ++c)
	{
		...
		{
			uint64_t pk = p ? mstl::bf::rotate_left(RandomHolding[c], p - 1) : 0;

			for (unsigned n = 0; n <= 4; ++n)
			{
				uint64_t nk = n ? mstl::bf::rotate_left(RandomHolding[2 + c], n - 1) : 0;

				for (unsigned b = 0; b <= 4; ++b)
				{
					uint64_t bk = b ? mstl::bf::rotate_left(RandomHolding[4 + c], b - 1) : 0;

					for (unsigned r = 0; r <= 4; ++r)
					{
						uint64_t rk = r ? mstl::bf::rotate_left(RandomHolding[6 + c], r - 1) : 0;
...
hash(Board const& board, variant::Type variant)
{
	...
	{
		// NOTE:
		// Not compatible to XBoard. The problem with the XBoard algorithm is:
		// 1) the description is quite unclear
		// 2) not the way how a Zobrist key will be generated; the chance is high
		//    that the XBoard method is generating colliding keys.
		...
		if (white.bishop)	key ^= mstl::bf::rotate_left(RandomHolding[5], white.bishop - 1);
		...
		if (black.bishop)	key ^= mstl::bf::rotate_left(RandomHolding[4], black.bishop - 1);
		if (black.rook)	key ^= mstl::bf::rotate_left(RandomHolding[6], black.rook   - 1);
		if (black.queen)	key ^= mstl::bf::rotate_left(RandomHolding[8], black.queen  - 1);
	}
...
static Move
decodeMove(Board const& board, uint16_t m, variant::Type variant)
{
	static piece::Type DecodePromoted[8] =
	{
		Queen,	// do not use None
		Knight,
		Bishop,
		Rook,
		Queen,
		King,		// extension for Antichess (forgotten in XBoard?)
		Queen,	// do not use None
		Queen,	// do not use None
	};

	// NOTE:
	// Not compatible to XBoard, the XBoard algorithm is a kludge
	// (hence not a standard).
	//
	// In our algorithm the dropped piece is decoded as promoted piece,
	// and the to-field is equal to from-field, thus a decoded move with
...
encodeMove(Move m)
{
	...
	// NOTE: We need an extension for Antichess, the promoted King
...
void
Book::MyEntry::fill(Byte const* src)
{
	util::ByteStream strm(const_cast<Byte*>(src), 16u);
...
void
Book::MyEntry::write(Byte* dst) const
{
	util::ByteStream strm(dst, 16u);
	...
	,m_used(0)
{
	...
	if (util::misc::file::suffix(filename) == "stm")
	{
		char buf[18];
		mstl::ifstream strm(sys::file::internalName(filename));
	...
	if (sys::file::size(filename) % 16)
		IO_RAISE(BookFile, Open_Failed, "'%s' has invalid file size", filename.c_str());

	sys::file::Mode mode = sys::file::Readable;

	if (sys::file::access(filename, sys::file::Writeable))
		mode = sys::file::Mode(mode | sys::file::Writeable);

	m_mapping = new Mapping(filename, mode);
...
bool
Book::isModified() const
{
	return m_someRemoved || m_someModified || (m_entryMap && m_used != m_entryMap->used());
}


bool
Book::readEntry(unsigned offset, MyEntry& entry)
{
	M_ASSERT(offset < m_bookSize);
	M_ASSERT(m_mapping);

	if (offset >= m_bookSize)
		return false;

	entry.fill(m_mapping->address() + ::mul16(offset));
	return true;
}
...
Book::findKey(uint64_t key)
{
	...
	else if (m_current.key == key)
	{
		return m_currentOffs;
	}
	else if (key <= m_currentOffs)
	{
		left = 0;
		right = m_currentOffs;
	}
	...
	while (left < right)
	{
		unsigned mid = ::div2(left + right);

		readEntry(m_currentOffs = mid, m_current);
	...
	if (left != m_currentOffs)
		readEntry(m_currentOffs = left, m_current);

	return m_current.key == key && m_current.move ? left : m_bookSize;
}
...
Book::probeMove(Board const& position, variant::Type variant, Choice choice)
{
	...
	if (m_entryMap)
	{
		if (Entry const* e = m_entryMap->find(key))
		{
...
bool
Book::probePosition(Board const& position, variant::Type variant, Entry& result)
{
	uint64_t key = ::hash(position, variant);
	...
	do
	{
		if (entry.move)
		{
			...
			item.move = decodeMove(position, entry.move, variant);
			item.weight = entry.weight;
		}
	}
	while (readEntry(++offs, entry) && entry.key == key);
...
Book::remove(Board const& position, variant::Type variant)
{
	...
	if (m_entryMap && m_entryMap->remove(key))
		return true;
...
{
	M_REQUIRE(isWriteable());
	M_REQUIRE(!entry.items.empty());

	uint64_t key = ::hash(position, variant);

	if (m_entryMap)
	{
		if (Entry* e = m_entryMap->find(key))
		{
			*e = entry;
			return true;
		}
...
Book::readScidMaskFile(mstl::ifstream& strm)
{
	...
	mstl::string sitem;
	mstl::string sentry;
	mstl::string scomment;
	mstl::string smove;
	mstl::string sannotation;
	mstl::string scolor;
	mstl::string smarker[2];
	...
	while (::readList(strm, sbuffer))
	{
		char const* s = sbuffer.begin();

		if (!::getList(sfen, s) || !::getList(svalues, s))
			return;

		if (board.setup(sfen, variant::Normal))
		{
			char const* s = svalues.begin();

			if (!::getList(sitem, s) || !::getList(scomment, s))
				return;

			uint64_t	key	= ::hash(board, variant::Normal);
			Entry&	e		= m_entryMap->find_or_insert(key, Entry());

			m_used = m_entryMap->used();
			e.comment = scomment;
			s = sitem.begin();

			while (::getList(sentry, s))
			{
				char const* s = sentry.begin();

				if (	!::getToken(smove, s)
					|| !::getToken(sannotation, s)
					|| !::getToken(scolor, s)
					|| !::getToken(scomment, s)
					|| !::getToken(smarker[0], s)
					|| !::getToken(smarker[1], s))
				{
					if (e.items.empty())
					{
						m_used = m_entryMap->used();
				...
				if (board.parseMove(smove, move, variant::Normal, move::MustBeUnambiguous))
				{
					e.items.push_back();

					Entry::Item& i = e.items.back();

					i.move = move;
					i.comment = scomment;
					i.info.annotation = nag::fromSymbol(sannotation);

					switch (scolor[0])
					{
						case 'G': i.info.color = Green; break;
						case 'Y': i.info.color = Yellow; break;
						case 'B': i.info.color = Blue; break;
						case 'R': i.info.color = Red; break;
					}

					for (unsigned k = 0; k < 2; ++k)
					{
						mstl::string const& s = smarker[k];

						if (!s.empty())
						{
							if (s == "::rep::_tb_include")
								i.info.include = 1;
							else if (s == "::rep::_tb_exclude")
								i.info.exclude = 1;
							else if (s == "::tree::mask::imageMainLine")
								i.info.mainline = 1;
							else if (s == "tb_bkm")
								i.info.bookmark = 1;
							else if (s == "::tree::mask::imageWhite")
								i.info.white = 1;
							else if (s == "::tree::mask::imageBlack")
								i.info.black = 1;
							else if (s == "tb_new")
								i.info.newline = 1;
							else if (s == "tb_rfilter")
								i.info.verify = 1;
