...
DatabaseCodec::~DatabaseCodec() noexcept
{
...
DatabaseCodec::makeCodec(mstl::string const& name, Mode mode)
{
	...
	switch (format::fromString(ext))
	{
		...
		case format::Scid5:
			return new si3::Codec(nullptr, true);
...
DatabaseCodec::getSuffixes(mstl::string const& filename, StringList& result)
{
	...
	switch (format::fromString(ext))
	{
		...
		case format::Scid5:			si3::Codec::getSuffixes(filename, result); break;
...
DatabaseCodec::rename(mstl::string const& oldName, mstl::string const& newName)
{
	...
	M_REQUIRE(format() == format::Scidb || format() == format::Scid3 || format() == format::Scid4 || format() == format::Scid5);
