...
// Codec for Scid formats: si3 (v300), si4 (v400), si5 (v500)
...
public:
	...
	Codec(CustomFlags* customFlags = 0, bool isVersion5 = false);
	~Codec() noexcept;
...
private:
	...
	void decodeIndexSi5(util::ByteStream& strm, unsigned index);
	void encodeIndexSi5(GameInfo const& item, unsigned index, util::ByteStream& buf);
	void readNamebasesSi5(mstl::string const& filename, util::Progress& progress);
	void writeNamebasesSi5(mstl::string const& filename);
