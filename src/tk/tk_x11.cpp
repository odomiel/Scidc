...
static int x11_ignore_badmatch_handler(Display* /*dpy*/, XErrorEvent* err) {
    x11_error_code = err->error_code;
    return 0;
}
		...
		&& attribs.visual->c_class == TrueColor)
	{
		...
		if (width > 0 && height > 0)
		{
			...
			XGrabPointer(	display,
								...
								CurrentTime);
#endif
			/* Ubuntu 24.04: catch BadMatch from XGetImage on Wayland/compositing WM */
			x11_error_code = 0;
			XErrorHandler oldHandler = XSetErrorHandler(x11_ignore_badmatch_handler);
			ximage = XGetImage(display, rootWindow, x, y, width, height, AllPlanes, ZPixmap);
			XSync(display, False);
			XSetErrorHandler(oldHandler);
			if (x11_error_code != 0) { ximage = 0; } /* BadMatch - ignore */
#if 0
			XUngrabPointer(display, CurrentTime);
#endif

			if (!ximage) { /* Ubuntu 24.04: XGetImage failed (Wayland/BadMatch) - skip */
				return TCL_OK;
			}
