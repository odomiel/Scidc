    ...
    const char *name)		/* Bind event to this resource (tag or image). */
{
	...
	*bindingTablePtr = Tk_CreateBindingTable(interp);
	...
	if (fifth[0] == '\0') {
	    return Tk_DeleteBinding(interp, *bindingTablePtr, static_cast<ClientData>( name, eventString);
	}
	if (fifth[0] == '+') {
	    fifth += 1;
	    append = true;
	}
	mask = Tk_CreateBinding(interp, *bindingTablePtr, static_cast<ClientData>( name, eventString, fifth, append);
	if (mask == 0) {
	    return TCL_ERROR;
	}
	if (mask & (unsigned) ~(motionMask|ButtonPressMask|ButtonReleaseMask|EnterWindowMask
	    ...
	    Tk_DeleteBinding(interp, *bindingTablePtr, static_cast<ClientData>( name, eventString);
	...
	command = Tk_GetBinding(interp, *bindingTablePtr, static_cast<ClientData>( name, Tcl_GetString(objv[0]));
	if (!command) {
	...
	Tk_GetAllBindings(interp, *bindingTablePtr, static_cast<ClientData>( name);
    }
    ...
    Tcl_HashEntry *hPtr)	/* Pointer into hash table, can be NULL. */
{
	...
	Tk_DeleteAllBindings(sharedTextPtr->tagBindingTable, static_cast<ClientData>( tagPtr->name);
    }
