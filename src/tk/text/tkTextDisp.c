    ...
    unsigned offset)
{
    ...
    if (TkTextTypePtrIsValid(segPtr)) { assert(segPtr->typePtr->group == SEG_GROUP_HYPHEN); } /* Ubuntu 24.04 compat */
    ...
    const TkTextDispChunk *chunkPtr)
{
    ...
    if (!chunkPtr->layoutProcs || !chunkPtr->clientData) { return '\0'; } /* Ubuntu 24.04 compat */
    ...
    const TkTextDispChunk *chunkPtr)
{
    ...
    ciPtr = chunkPtr->clientData;
    if (!chunkPtr->clientData || !ciPtr || ciPtr->numBytes == 0) { return '\0'; } /* Ubuntu 24.04 compat */
    ...
    TkText *textPtr)		/* Overall information for text widget. */
{
	...
	Tcl_CancelIdleCall(RunUpdateLineMetricsFinished, static_cast<ClientData>( textPtr);
    }
    ...
    int byteOffset)
{
    ...
    if (!segPtr->typePtr->layoutProc) { return false; } /* Ubuntu 24.04 compat */
    ...
    DLine *dlPtr)
{
	...
	if (!dispLineInfo) { goto skipDispLineEntry; } /* Ubuntu 24.04 compat: NULL check */
    ...
    skipDispLineEntry: ; /* Ubuntu 24.04 compat: jump here if dispLineInfo is NULL */
    ...
    unsigned displayLineNo)	/* Display line number of logical line, needed for caching. */
{
    ...
    if (data.numBytesSoFar == 0) {
	/* Ubuntu 24.04 compat: layoutProc NULL due to Tk symbol conflict.
	 * Create a minimal elide chunk with a valid section so GetBbox works. */
	LayoutMakeNewChunk(&data);
	data.chunkPtr->layoutProcs = &layoutElideProcs;
	...
	dlPtr->byteCount = 1;
	/* Assign a section so chunkPtr->sectionPtr is never NULL */
	if (dlPtr->chunkPtr && !dlPtr->chunkPtr->sectionPtr) {
	    TkTextDispChunkSection *sect = LayoutNewSection(textPtr->dInfoPtr);
	    sect->chunkPtr = dlPtr->chunkPtr;
	    sect->numBytes = 1;
	    dlPtr->chunkPtr->sectionPtr = sect;
	}
	LayoutUpdateLineHeightInformation(&data, dlPtr, data.logicalLinePtr, true, 0, 0, false);
	return dlPtr;
    }
    ...
    TkText *textPtr)
{
	...
	if (!textPtr->dInfoPtr->pendingUpdateLineMetricsFinished) {
	    ...
	    Tcl_DoWhenIdle(RunUpdateLineMetricsFinished, static_cast<ClientData>( textPtr);
	}
    ...
    int lastOffset)
{
	...
	case SEG_GROUP_WINDOW: {
	    ...
	    if (segPtr->typePtr->layoutProc && segPtr->typePtr->layoutProc(&index, segPtr, 0, maxX, 1, false, data->wrapMode,
