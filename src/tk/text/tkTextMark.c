...
const Tk_SegType __attribute__((visibility("hidden"))) tkTextRightMarkType = {
    "mark",		/* name */
...
const Tk_SegType __attribute__((visibility("hidden"))) tkTextLeftMarkType = {
    "mark",		/* name */
    SEG_GROUP_MARK,	/* group */
    ...
    TkTextUndoInfo *undoInfo)		/* Undo information, can be NULL */
{
    ...
    assert(markPtr);
    /* Ubuntu 24.04 compat: if typePtr invalid (system Tk), skip */
    if (!TkTextTypePtrIsValid(markPtr)) { return; }
