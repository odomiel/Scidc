...
int __attribute__((visibility("hidden"))) tkBTreeDebug = false;
...
const Tk_SegType __attribute__((visibility("hidden"))) tkTextCharType = {
    "character",		/* name */
...
const Tk_SegType __attribute__((visibility("hidden"))) tkTextHyphenType = {
    "hyphen",			/* name */
...
const Tk_SegType __attribute__((visibility("hidden"))) tkTextBranchType = {
    "branch",			/* name */
...
const Tk_SegType __attribute__((visibility("hidden"))) tkTextLinkType = {
    "connection",		/* name */
    SEG_GROUP_BRANCH,		/* group */
...
const Tk_SegType __attribute__((visibility("hidden"))) tkTextProtectionMarkType = {
    "protection",		/* name */
    ...
    const TkTextUndoToken *item)
{
    ...
    while (segPtr) {
	/* assert(segPtr->typePtr->inspectProc); -- disabled Ubuntu 24.04 compat */
    ...
    bool reused)
{
	...
	if (numSegments > 0) {
		...
		while (segPtr) {
		    ...
		    if (!segPtr->typePtr->deleteProc) { segPtr = nextPtr; continue; } /* Ubuntu 24.04 compat */

		    nextPtr = (segPtr->nextPtr && !segPtr->sectionPtr) ? segPtr->nextPtr : NULL;
		    if (segPtr->typePtr->deleteProc) { segPtr->typePtr->deleteProc(sharedTextPtr, segPtr, DELETE_BRANCHES | DELETE_MARKS); } /* Ubuntu 24.04 compat */
		    segPtr = nextPtr;
		}
	    }
...
LinkMark(
    ...
    TkTextSegment *segPtr)
{
    /* Ubuntu 24.04 compat: if typePtr invalid, skip linking this segment */
    if (!TkTextTypePtrIsValid(segPtr)) { return; }
    ...
    TkTextSegment *succPtr)	/* Link this segment after predPtr. */
{
    ...
    assert(succPtr);
    TkTextRepairTypePtr(succPtr); /* Ubuntu 24.04 compat */
    if (TkTextTypePtrIsValid(succPtr)) { assert(succPtr->typePtr->group == SEG_GROUP_BRANCH); } /* Ubuntu 24.04 compat */
    ...
    TkTextUndoInfo *undoInfo)	/* Store undo information, can be NULL. */
{
    ...
    /* Ubuntu 24.04 compat: validate typePtr - system Tk has different Tk_SegType layout */
    if (!TkTextTypePtrIsValid(firstSegPtr) || !TkTextTypePtrIsValid(lastSegPtr)) {
	/* typePtr points to system Tk structure - cannot safely proceed */
	return;
    }
	...
	if (!segPtr) {
	    ...
	    if (segPtr->typePtr->deleteProc && !segPtr->typePtr->deleteProc(sharedTextPtr, segPtr, flags)) {
		assert(segPtr->typePtr); /* really still living? */
		TkTextRepairTypePtr(segPtr); /* Ubuntu 24.04 compat */
		if (TkTextTypePtrIsValid(segPtr)) { assert(segPtr->typePtr->group == SEG_GROUP_MARK
			|| segPtr->typePtr->group == SEG_GROUP_BRANCH); } /* Ubuntu 24.04 compat */
		...
		if (prevLinkPtr && segPtr->typePtr == &tkTextBranchType) {
			...
			TkTextRepairTypePtr(segPtr); /* Ubuntu 24.04 compat */
			if (TkTextTypePtrIsValid(segPtr)) { assert(segPtr->typePtr->group == SEG_GROUP_MARK); } /* Ubuntu 24.04 compat */
	...
	TkTextRepairTypePtr(firstSegPtr); TkTextRepairTypePtr(lastSegPtr); /* Ubuntu 24.04 compat */
	if (TkTextTypePtrIsValid(firstSegPtr)) { assert(firstSegPtr->typePtr->group & (SEG_GROUP_MARK|SEG_GROUP_PROTECT)); } /* Ubuntu 24.04 compat */
	if (TkTextTypePtrIsValid(lastSegPtr)) { assert(lastSegPtr->typePtr->group & (SEG_GROUP_MARK|SEG_GROUP_PROTECT)); } /* Ubuntu 24.04 compat */
	...
	if (!TkTextIsSpecialOrPrivateMark(firstSegPtr)) {
	    ...
	    /* assert(firstSegPtr->typePtr->deleteProc); -- disabled Ubuntu 24.04 compat */
	    if (firstSegPtr->typePtr->deleteProc && !firstSegPtr->typePtr->deleteProc(sharedTextPtr, firstSegPtr, flags)) {
	...
	if (!TkTextIsSpecialOrPrivateMark(lastSegPtr)) {
	    ...
	    /* assert(lastSegPtr->typePtr->deleteProc); -- disabled Ubuntu 24.04 compat */
	    if (lastSegPtr->typePtr->deleteProc && !lastSegPtr->typePtr->deleteProc(sharedTextPtr, lastSegPtr, flags)) {
		assert(!"mark refuses to die"); /* this should not happen */
    ...
    TkTextUndoInfo *redoInfo)	/* Store undo information, can be NULL. */
{
    ...
    /* Ubuntu 24.04 compat: ensure protection marks are unlinked before use */
    if (sharedTextPtr->protectionMark[0]->sectionPtr) {
	UnlinkSegment(sharedTextPtr->protectionMark[0]);
    }
    if (sharedTextPtr->protectionMark[1]->sectionPtr) {
	UnlinkSegment(sharedTextPtr->protectionMark[1]);
    }
    				 ...
    				 * indicated here. */
{
    ...
    TkTextRepairTypePtr(segPtr); /* Ubuntu 24.04 compat */
    if (TkTextTypePtrIsValid(segPtr)) { assert(segPtr->typePtr->group != SEG_GROUP_CHAR); } /* Ubuntu 24.04 compat */
    if (TkTextTypePtrIsValid(segPtr)) { assert(segPtr->typePtr->group != SEG_GROUP_PROTECT); assert(segPtr->typePtr->group != SEG_GROUP_BRANCH); } /* Ubuntu 24.04 compat */
    ...
    TkTextSegment *segPtr)	/* Segment to be unlinked. */
{
    ...
    TkTextRepairTypePtr(segPtr); /* Ubuntu 24.04 compat */
    if (TkTextTypePtrIsValid(segPtr)) { assert(segPtr->typePtr->group != SEG_GROUP_PROTECT); } /* Ubuntu 24.04 compat */
    if (TkTextTypePtrIsValid(segPtr)) { assert(segPtr->typePtr->group != SEG_GROUP_BRANCH); } /* Ubuntu 24.04 compat */
    ...
    TkTextSegment **segPtr2)
{
	...
	*segPtr1 = linePtr1->segPtr;
    ...
    /* Ubuntu 24.04 compat: ensure protection mark is unlinked before use */
    if (sharedTextPtr->protectionMark[0]->sectionPtr) {
	UnlinkSegment(sharedTextPtr->protectionMark[0]);
    }
