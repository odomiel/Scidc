...
MODULE_SCOPE __attribute__((visibility("hidden"))) int tkBTreeDebug;
MODULE_SCOPE __attribute__((visibility("hidden"))) int tkTextDebug;
MODULE_SCOPE __attribute__((visibility("hidden"))) const Tk_SegType tkTextCharType;
MODULE_SCOPE __attribute__((visibility("hidden"))) const Tk_SegType tkTextBranchType;
MODULE_SCOPE __attribute__((visibility("hidden"))) const Tk_SegType tkTextLinkType;
MODULE_SCOPE __attribute__((visibility("hidden"))) const Tk_SegType tkTextLeftMarkType;
MODULE_SCOPE __attribute__((visibility("hidden"))) const Tk_SegType tkTextRightMarkType;

/* Ubuntu 24.04 compat: check if typePtr is valid (not from system Tk) */
static inline int TkTextTypePtrIsValid(const TkTextSegment *segPtr) {
    if (!segPtr || !segPtr->typePtr) return 0;
    unsigned g = (unsigned)segPtr->typePtr->group;
    /* Valid group: exactly one bit set between 1<<0 and 1<<7 */
    return (g != 0 && g <= (1<<7) && (g & (g-1)) == 0);
}
/* No-op repair - we no longer mutate typePtr as it can corrupt segment data */
static inline void TkTextRepairTypePtr(TkTextSegment *segPtr) {
    (void)segPtr; /* intentionally empty - see TkTextTypePtrIsValid */
}
MODULE_SCOPE __attribute__((visibility("hidden"))) const Tk_SegType tkTextHyphenType;
MODULE_SCOPE __attribute__((visibility("hidden"))) const Tk_SegType tkTextEmbImageType;
MODULE_SCOPE __attribute__((visibility("hidden"))) const Tk_SegType tkTextEmbWindowType;
MODULE_SCOPE __attribute__((visibility("hidden"))) const Tk_SegType tkTextProtectionMarkType;
