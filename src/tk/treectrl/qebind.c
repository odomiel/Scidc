...
int QE_InstallEvent(QE_BindingTable bindingTable, char *name, QE_ExpandProc expandProc)
{
	...
	Tcl_SetHashValue(hPtr, static_cast<ClientData>( eiPtr);
...
int QE_InstallDetail(QE_BindingTable bindingTable, char *name, int eventType, QE_ExpandProc expandProc)
{
	...
	Tcl_SetHashValue(hPtr, static_cast<ClientData>( dPtr);
...
void QE_DeleteBindingTable(QE_BindingTable bindingTable)
{
	...
	while (hPtr != NULL)
	{
		...
		Tk_DeleteEventHandler(cd->tkwin, StructureNotifyMask,
			TkWinEventProc, static_cast<ClientData>( cd);
		Tcl_Free((char *) cd);
		hPtr = Tcl_NextHashEntry(&search);
	}
	...
	char *eventString, char *command, int append)
{
	...
	if (isNew)
	{
			...
			if (tkwin2 != NULL)
			{
				...
				if (isNew)
				{
					...
					Tk_CreateEventHandler(tkwin2, StructureNotifyMask,
						TkWinEventProc, static_cast<ClientData>( cd);
					Tcl_SetHashValue(hPtr, static_cast<ClientData>( cd);
				}
				else
				{
		...
		Tcl_SetHashValue(hPtr, static_cast<ClientData>( valuePtr);
	}
	...
	EventInfo *eiPtr, Detail *dPtr, GenerateData *gdPtr)
{
		...
		valuePtr; valuePtr = valuePtr->nextValue)
	{
		...
		if (command != NULL)
		{
			...
			data.eventPtr = eiPtr;
			data.detailPtr = dPtr;
			eventPtr->clientData = static_cast<ClientData>( &data;
		...
		else if (gdPtr != NULL)
		{
			 ...
			 */
			eventPtr->clientData = static_cast<ClientData>( gdPtr;
			ExpandPercents(bindPtr, valuePtr->object, valuePtr->command,
	...
	Tcl_Obj *CONST objv[])
{
	...
	if (string[0] == '.')
	{
		...
		object = static_cast<ClientData>( Tk_GetUid(Tk_PathName(tkwin2));
	}
	else
	{
		object = static_cast<ClientData>( Tk_GetUid(string);
	}
	...
	Tcl_Obj *CONST objv[])
{
	...
	{
		object = static_cast<ClientData>( Tk_GetUid(t);
	}
