...
proc dbNew {parent variant} {
	...
	lappend filetypes [list $mc::ScidbBases {.sci}]
	if {$variant in {Normal Undetermined}} {
		lappend filetypes [list $mc::PGNFilesArchives {.pgn .pgn.gz}]
	}
	set defaultExtension [expr {$variant eq "Normal" ? ".si5" : ".sci"}]
...
#		[list $mc::AllScidbFiles	{.sci .si5 .si4 .si3 .cbh .cbf .scv .pgn .pgn.gz .bpgn .bpgn.gz .zip} \
#											{.CBF .PGN .ZIP}]
#		[list $mc::BPGNFilesArchives	{.bpgn .bpgn.gz .zip} {.ZIP}]
	set filetypes [list                                                                               \
		[list $mc::AllScidbFiles	{.sci .si5 .si4 .si3 .cbh .cbf .scv .pgn .pgn.gz .zip} {.CBF .PGN .ZIP}] \
		[list $mc::AllScidbBases	{.sci .si5 .si4 .si3 .cbh .cbf .scv} {.CBF}]                             \
		[list $mc::ScidbBases		{.sci}]                                                             \
		[list $mc::ScidBases			{.si5 .si4 .si3}]                                                        \
	...
	set result [::dialog::openFile \
		...
		-customtooltip $mc::Archiving \
		-customcommand [namespace code [list CreateArchive]] \
		-customfiletypes {.sci .si5 .si4 .si3 .cbh .cbf .pgn .pgn.gz .bpgn .bpgn.gz .zip .CBF .PGN .ZIP} \
	]
	set FileSelBoxInUse 0
...
#	switch $ext {
#		.sci - .si3 - .si4 - .si5 - .cbh - .cbi	{ return $mc::Data(index) }
