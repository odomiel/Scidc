...
proc GetFolders {w} {
	...
	switch [tk windowingsystem] {
		x11 {
			# Ubuntu 24.04 compat: scan /media and /run/media directly
			foreach mp [glob -nocomplain -directory /media -types d *] {
				set id [file tail $mp]
				if {[string length $id] == 0} continue
				lappend folders $id
				lappend Vars(usb-devices) $id
				set idl [string tolower $id]
				set Vars(folder:$idl) $mp
				set Vars(icon:$idl) $icon::16x16::usb
				set Vars(lookup:$mp) usb
				lappend Vars(folders) $mp
			}
			foreach mp [glob -nocomplain -types d /run/media/*/*] {
				set id [file tail $mp]
				if {[string length $id] == 0} continue
				lappend folders $id
				lappend Vars(usb-devices) $id
...
proc DetailedLayout {w} {
	...
	if {$Vars(type) eq "dir"} {
		...
		set Vars(scriptFile) {
			...
			set size ""
			if {[catch {{*}$Vars(sizecommand) $file $mtime} size]} { set size "" }
			set valid 1
			if {[catch {{*}$Vars(validatecommand) $file $size} valid]} { set valid 1 }
			if {$valid} {
				set valid 1
				set item [$t item create -open no]
				$t item style set $item name styName size stySize modified styDate
...
proc ListLayout {w} {
	...
	set Vars(scriptFile) {
		set valid 1
		if {[catch {{*}$Vars(validatecommand) $file} valid]} { set valid 1 }
		if {$valid} {
			set valid 1
			set item [$t item create -open no]
			$t item style set $item name styName
