...
proc Select {path base variant index} {
	...
	set position [::scidc::db::get lookupPlayer $index $Vars($base:$variant:view) $base $variant]
...
proc InitBase {path base variant} {
	...
	if {![info exists Vars($base:$variant:view)]} {
		...
		set Vars($base:$variant:view) \
			[::scidc::view::new $base $variant master slave slave slave slave slave]
		set Vars($base:$variant:update:players) 1
...
proc Update2 {id path base variant} {
	...
	if {$selected >= 0 && $lastChange < $Vars($base:$variant:lastChange)} {
		if {[llength $Vars($base:$variant:selected:key)]} {
			set index [::scidc::db::find player $base $variant $Vars($base:$variant:selected:key)]
			if {$index >= 0} {
				set selected [::scidc::db::get lookupPlayer $index $view $base $variant]
		...
		set n [::scidc::view::count events $base $variant $view]
...
proc Search {path base variant view {selected -1}} {
	...
	if {$selected >= 0} {
		...
		set Vars($base:$variant:lastChange) [::scidc::db::get lastChange $base $variant]
		::scidc::view::search $base $variant $view null events [list player $selected]
...
proc DoUpdate {path base variant} {
	...
	if {$Vars(active)} {
		...
		if {$Vars($base:$variant:update:players)} {
			set n [::scidc::db::count players $base $variant]
